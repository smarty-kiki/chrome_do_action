...
| `list_elements` | `send <id> list_elements <tab> '{"filter":"upload","visible":true}'` | **页面元素地图**：列出可交互元素（生成好的 selector/可见性/坐标/accept 等），支持 filter/text/max/visible；穿透 shadow DOM，缺省聚合所有 frame（元素带 frame url）；找不到元素先查它。加 `{"closed":true}` 连**封闭型 shadow 子树**内的元素一起列（带 `backendNodeId` 无 `selector`，排在列表最前） |
...
3. **要坐标用 `get_rect`，不要从截图像素反推**：`get_rect '{"text":"发布","exact":true}'` 直接给权威 `centerCss`，与 `real_click {x,y}` 同一坐标口径（顶层视口 CSS px，iframe 内元素已加好 frame 偏移）。**别再从截图找色块、猜缩放/偏移反推坐标**——那种做法绑死"页面上恰好有个位置固定的参照物"，页面一改版就整体失效。`screenshot` 也返回换算元数据（`{imagePx, viewportCss, dpr, chromeInsetCss, scrollCss, mapping}`，`css = imagePx / dpr`），但它用于**看页面真实状态**，不是取坐标的手段。
   - **`real_click` 坐标点击后核对 `hit`**：返回里的 `hit` 是它**实际点到**的元素（`{tag, class, text, backendNodeId}`，在按下鼠标之前采样）——`hit.text` 不是目标就立即中止。给坐标却静默点到旁边的按钮（如「暂存离开」这类中断流程的按钮）是真实事故，靠这个字段挡掉。同时返回 `navigated`（URL 是否变了）和 `settledMs`。"点了却没反应"时先看 `warning`：它只在链路里某步**非致命但没走完**时出现（窗口不可聚焦导致激活超时、鼠标轨迹没走完），原因写在值里——否则这种情况只能瞎猜。
   ...
   - **坐标只在本命令内闭环，别跨命令搬运**：部分命令执行期间浏览器可视区域会临时变矮一条（Chrome 会浮出一条提示条），所以 `get_viewport`、`screenshot.viewportCss`、`list_elements` 的坐标可能不是同一个视口口径——**别拿一个去校验或补偿另一个**。要动手就二选一：① 用 `get_rect` 拿 `centerCss` 后立刻 `real_click` 用同一份坐标（这两者同一口径）；② 更稳的是直接给 `selector`/`text`/`backendNodeId`，让命令自己量自己点。若返回里出现 `viewportNote`，说明这次测量与平时的视口口径不同，别拿它跟别的命令对齐。
...
6. **shadow DOM 自动穿透**：Web Components 站点（小红书后台等）的按钮/编辑器在 shadow root 里，普通选择器找不到。三种方式：① 直接粘贴 DevTools 元素路径（含 `#shadow-root` 标记）；② `selector: "xhs-publish-btn >>> button"`（`>>>` 穿透所有层）；③ 裸选择器/xpath/text 会自动兜底搜索 open shadow root。**封闭型 shadow 子树**（页面上看得见、但普通查询一律报"不存在"的按钮/输入框，常见于页脚操作栏）则走 `backendNodeId` 通道：`list_elements '{"closed":true}'` 枚举（带 `backendNodeId`，无 `selector`）→ `get_rect '{"backendNodeId":N}'` 量矩形 → `click`/`real_click`/`get_prop`/`get_text` 传 `{"backendNodeId":N}` 操作；能用 `backendNodeId` 的就这五个命令，`type`/`keyboard`/`trigger`/`upload_*` 传它会**明确报错**（这类元素给不出选择器，给不出就不假装成功）；`get_rect` 对普通查询也会自动兜底到这类子树（无需开关）。这类元素连页面里注入的任意 JS 都看不见，别指望用 `exec` 绕。`get_page_info --field html` 默认含 open shadow 内容（`<template shadowrootmode="open">` 内联），但**不含封闭的**。
...
11. **--field 精确取结果**：所有返回对象的命令都支持 `--field` 点路径裁剪：`--field "clickDesc.selector,settledMs,currentTab.url"` → `{clickDesc:{selector},settledMs,currentTab:{url}}`；`--field "newTabs.url"` → `{newTabs:[url,...]}`（数组返回同样按项裁剪，如 `list_tabs --field url`）。只取需要字段，减少输出、加快响应（不采集未请求的页面信息）。字段名拼错时 CLI 会在 stderr 明确提示"这个字段没匹配到"，其余字段照常返回——不会静默给个空。`get_text` 返回纯文本、`get_prop` 标量值原样返回——均无字段可滤（`get_prop` 对象值可裁剪）。
...
14. **错误码能区分失败类别**：CLI 打印 `Error [code]: message`。`not-found` = 真的不存在（换 selector 或先 list_elements，报错里会附一句下一步该做什么）；`unreachable-subtree` = **存在但不可寻址**（多半在封闭型 shadow 子树里，改用 `backendNodeId` 通道，见技巧 6）；`cdp-unavailable` = 调试通道被占用（最常见的原因是 DevTools 开着），关掉 DevTools 重试。不要把三者当成同一件事盲目重试。元素被遮挡**不是**错误，是 `get_rect`/`clickDesc` 返回里的 `covered`/`coveredBy`。
15. **读到的文字 = 页面上的文字**：`get_text`、`get_rect.text`、`list_elements[].text`、`clickDesc` 里的文字全部**逐字原样**带出——含前后空格与换行，不 trim、不折叠、不截断、不改写。查询（`text` 定位/过滤）则放宽为「原样包含 **或** 空白折叠后包含」任一成立即命中：页面上的文字照抄能查到，多几个空格也不影响命中，但**返回给你看的永远是原样那份**。
16. **每条命令都串行等结果**：命令返回即代表本次动作的影响已经落地（DOM 稳定、按钮已生效），不存在"发出去就返回、页面还在变"的情况；动作停在中间态时会继续等（`waitStableMs` 可拿到停稳后的终态）。**边界**：动作后**超过约 1 秒**才发生的页面变化（服务端请求回来后渲染、长 debounce、纯网络等待）不在覆盖内——这类用 `waitFor` 谓词显式等，或把 tab 切前台再重试。
...
- 部分命令执行期间 Chrome 顶部会闪过一条提示条（正常现象）；它会让可视区域临时变矮一条，所以别跨命令搬运坐标（见技巧 3）
- **不要用 `exec` 跑任意 JS 当通用手段**：常规流程用具体命令（click/type/get_prop/list_elements/screenshot 等）足够；只有排查页面问题时才考虑 `exec`，且必须先确认插件配置已勾选「允许 exec 命令」——它在页面里执行任意代码，能读取/修改页面一切数据，仅排查问题临时开启，用完关闭
