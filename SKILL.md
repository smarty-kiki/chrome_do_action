...
8. **富文本输入**：用 `type` 输入纯文本时文字**原样插入、不自动分段**——要分段就逐段发（后续 `mode:"append"` 追加）；要带格式的排版直接用 `paste_rich` 粘贴排好版的 HTML——块级结构由编辑器自己的粘贴处理分段解析，返回值 `pipeline:"editor_paste"` 即编辑器已接管（`"insertHTML_fallback"` 则说明目标没接管，可能不分段）。cda 不做任何编辑器嗅探/适配，最终效果由编辑器自己决定，某种编辑器行不行由使用者自行验证。
