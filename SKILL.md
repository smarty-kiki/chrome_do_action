...
8. **富文本输入**：用 `type` 输入纯文本时文字**原样插入、不自动分段**——要分段就逐段发（后续 `mode:"append"` 追加）；要带格式的排版直接用 `paste_rich` 粘贴排好版的 HTML。cda 不做任何编辑器适配——文字/HTML 原样交给页面，最终效果由编辑器自己决定，某种编辑器行不行由使用者自行验证。
