...
- 粘贴行为：富文本编辑器收到程序化粘贴后按自己的粘贴处理解析 HTML——**块级结构（段落/标题/列表/表格）由编辑器自己分段**，而不是整段糊在一起；没有粘贴处理的页面（如纯 contenteditable）则原样插入。具体呈现因编辑器而异，cda 不做任何编辑器嗅探/适配/清洗——怎么适应是调用方 agent 的事
- 返回值 `pipeline` 标识本次内容被谁处理：`editor_paste` = 编辑器的粘贴处理接管了（富文本编辑器上的预期值，分段解析在编辑器内完成）；`insertHTML_fallback` = 目标没有接管粘贴（纯 contenteditable 属正常；富文本编辑器出现此值说明它没响应程序化粘贴，此时块级结构可能不分段）
