...
function returnedRootKeys(data: unknown): string[] | null {
...
function fieldMisses(data: unknown, fields: string[]): string[] {
...
const cmdName = (msg as { payload?: { command?: string } }).payload?.command;
const cmdParams = ((msg as { payload?: { params?: Record<string, unknown> } }).payload?.params || {}) as Record<string, unknown>;
...
ws.on("message", (raw: Buffer) => {
  ...
  if (res.type === "error") {
    console.error(`Error: ${(res.payload as { message?: string } | undefined)?.message || "unknown"}`);
    process.exit(1);
  }

  if (!(res.type === "cli_result" && res.payload)) return; // 无关消息忽略，继续等结果
